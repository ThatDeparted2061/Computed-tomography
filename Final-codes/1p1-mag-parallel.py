...
def visualize_sinogram(projections, slice_indices_to_show=[0]):
    """
    Visualizes the sinogram for specified slices.
    A sinogram is a 2D image where the x-axis is the detector position
    and the y-axis is the projection angle.
    """
    num_slices, num_angles, detector_width = projections.shape

    # Determine how many sinograms to show, up to a maximum for readability
    if len(slice_indices_to_show) == 0:
        print("No slice indices provided for sinogram visualization.")
        return

    # Limit the number of sinograms displayed to avoid overcrowding
    max_plots_per_figure = 5
    if len(slice_indices_to_show) > max_plots_per_figure:
        print(f"Displaying sinograms for the first {max_plots_per_figure} requested slices (out of {len(slice_indices_to_show)}).")
        indices_to_display = slice_indices_to_show[:max_plots_per_figure]
    else:
        indices_to_display = slice_indices_to_show

    rows = 1
    cols = len(indices_to_display)
    
    plt.figure(figsize=(4 * cols, 5 * rows))
    plt.suptitle("Sinogram Visualization", y=1.02)

    for i, slice_idx in enumerate(indices_to_display):
        if 0 <= slice_idx < num_slices:
            plt.subplot(rows, cols, i + 1)
            # Sinogram: x-axis = detector position, y-axis = angle
            # Projections are (slices, angles, detector_width)
            # So, for a single slice, projections[slice_idx] is (angles, detector_width)
            # imshow will naturally plot this with angles on y-axis and detector_width on x-axis
            plt.imshow(projections[slice_idx], cmap='gray', aspect='auto', 
                       extent=[0, detector_width, 0, num_angles]) # Extent for better axis labeling
            plt.title(f'Sinogram for Slice {slice_idx}')
            plt.xlabel('Detector Position (pixels)')
            plt.ylabel('Projection Angle (index)')
            plt.colorbar(label='Intensity')
        else:
            print(f"Warning: Slice index {slice_idx} is out of bounds (0 to {num_slices-1}). Skipping.")
...
def preprocess_projection_data(projections, apply_alignment=True):
    ...
    if apply_alignment:
        ...
        # This is a placeholder; real alignment requires more sophisticated algorithms
        # For demonstration, we'll just show it as a potential step.
        # It's difficult to implement a robust general alignment without specific knowledge
        # of the misalignment type.
        pass # No actual alignment implemented for simplicity in this example
...
def save_all_slices(reconstruction, output_dir, format='png'):
    """Save all reconstructed slices as images"""
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    print(f"\nSaving all {reconstruction.shape[0]} slices to {output_dir}...")
    for i in range(reconstruction.shape[0]):
        filename = os.path.join(output_dir, f'slice_{i:04d}.{format}')
        plt.imsave(filename, reconstruction[i], cmap='gray')
        
        if i % 50 == 0 or i == reconstruction.shape[0]-1:
            print(f"\rProgress: {i+1}/{reconstruction.shape[0]} slices saved", end="", flush=True)
...
def reconstruct_and_visualize(projections, angles):
    ...
    # Visualize sinogram for a few slices
    # Choose a few representative slices to display their sinograms
    sinogram_slices_to_show = [0, height//4, height//2] 
    visualize_sinogram(projections, sinogram_slices_to_show)
...
def main():
    ...
    slices_output_dir = os.path.join(output_dir, 'all_slices')
    ...
    try:
        ...
        # Reconstruct with enhanced visualization (now includes sinogram viz)
        reconstruction = reconstruct_and_visualize(projections, angles)
        
        # Save results
        save_reconstruction(reconstruction, output_dir)
        
        # Save all slices after closing plots
        save_all_slices(reconstruction, slices_output_dir)
