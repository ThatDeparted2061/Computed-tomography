...
def load_specific_projection_files(file_pattern, min_file_num, max_file_num,
                                   ...
                                   extend_to_360=False, num_last_slices_to_copy=20):
    """
    ...
    Args:
        ...
        num_slices (int, optional): Number of slices to select for reconstruction.
                                    ...
                                    Defaults to None.
        extend_to_360 (bool, optional): If True, extends the dataset to 360 slices by copying
                                        the last num_last_slices_to_copy slices repeatedly.
                                        Defaults to False.
        num_last_slices_to_copy (int, optional): Number of slices to copy from the end when extending.
                                              Defaults to 20.
    ...
    try:
        ...
        # Extend to 360 slices if requested
        if extend_to_360 and num_slices_to_use == 181:
            print(f"\nExtending dataset to 360 slices by copying last {num_last_slices_to_copy} slices...")
            
            # Calculate how many times we need to repeat the last slices
            slices_needed = 360 - 181
            repeat_times = int(np.ceil(slices_needed / num_last_slices_to_copy))
            
            # Get the last num_last_slices_to_copy slices
            last_slices = projections[-num_last_slices_to_copy:, :, :]
            
            # Repeat these slices as needed
            extended_slices = np.tile(last_slices, (repeat_times, 1, 1))[:slices_needed]
            
            # Concatenate with original projections
            projections = np.concatenate([projections, extended_slices], axis=0)
            
            print(f"Original number of slices: 181")
            print(f"Extended with {slices_needed} additional slices")
            print(f"New number of slices: {projections.shape[0]}")
            print(f"New center slice index: {projections.shape[0]//2}")
...
def main():
    ...
    params = {
        ...
        'min_file_num': 1,
        'max_file_num': 360,
        'num_slices_to_reconstruct': 181,  # Will be extended to 360
        'start_angle': 0,
        'end_angle': 360,
        'source_to_detector': 800.0,
        'source_to_object': 488.0,
        ...
        'sirt_iterations': 50,
        'extend_to_360': True,  # New parameter to enable extension to 360 slices
        'num_last_slices_to_copy': 20  # Number of slices to copy from the end
    }
    ...
    try:
        ...
        projections, angles = load_specific_projection_files(
            ...
            extend_to_360=params['extend_to_360'],
            num_last_slices_to_copy=params['num_last_slices_to_copy']
        )
