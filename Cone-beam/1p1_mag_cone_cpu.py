...
# Suppress ASTRA warnings if they are too verbose (uncomment if warnings are excessive)
...
def reconstruct_cone_beam(projections, angles, proj_geom, vol_geom, algorithm_type='FDK_CPU', num_iterations=50):
    ...
    This version is modified to prefer CPU algorithms.
    ...
    if algorithm_type == 'FDK_CPU':
        cfg = astra.astra_dict('FDK_CPU') # Changed from FDK_CUDA to FDK_CPU
    ...
    elif algorithm_type == 'SIRT3D_CPU': # Changed from SIRT3D_CUDA to SIRT3D_CPU
        cfg = astra.astra_dict('SIRT3D_CPU') # Ensure CPU version is selected
        cfg['ReconstructionDataId'] = rec_id
        cfg['ProjectionDataId'] = proj_id
        # For iterative algorithms, you often need a projector
        # Changed from 'cuda3d' to 'fp3d' for CPU forward projector
        projector_id = astra.create_projector('fp3d', proj_geom, vol_geom)
        ...
        raise ValueError(f"Unsupported algorithm type: {algorithm_type}. Choose 'FDK_CPU' or 'SIRT3D_CPU'.")
...
def main():
    ...
    print("           (CPU ACCELERATED)                               ") # Added CPU indication
    print("=" * 70)
    ...
    params = {
        ...
        'output_dir': 'cone_beam_reconstruction_results_cpu', # Changed output directory name
        ...
        'reconstruction_algorithm': 'FDK_CPU', # Changed default to FDK_CPU
    ...
    try:
        ...
        print(f"\n[CRITICAL] An unhandled error occurred during main execution: {e}")
        import traceback
        traceback.print_exc()
